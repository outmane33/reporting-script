...
// Paths to files
const accountsFilePath = path.join(__dirname, "yahoo_accounts.txt");
...
const processAccount = async (account, proxy) => {
  ...
  const config = {
    imap: {
      user: email,
      password,
      host: "imap.mail.yahoo.com", // Yahoo's IMAP server
      port: 993,
      tls: true,
  ...
  try {
    ...
    await connection.openBox("Bulk"); // Yahoo's Spam folder
    ...
    const targetMessage = messages.find((message) => {
      ...
      return subject === "hello world" && from.includes("roushajar@gmail.com");
    });
    ...
    await connection.moveMessage(uid, "Inbox"); // Yahoo's Inbox
